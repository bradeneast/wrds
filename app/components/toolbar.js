...
export default () => {
    ...
    <div class=configuration id=undo_redo>
        ...
        data-icon
        title=Redo
        ?disabled=${State.nowIndex >= State.history.length - 1}
        @click=${NavigateHistory.bind(this, 1)}>
        </button>
    ...
    <div class=configuration id=zoom>
        ...
        data-icon
        title="Zoom Out"
        @click=${misc.zoom.bind(this, 'out')}>
        </button>

        <button
        data-icon
        title="Zoom In"
        @click=${misc.zoom.bind(this, 'in')}>
        </button>
    ...
    <div class=configuration>
        ...
        <button 
        data-icon 
        title=${"You're in " + (settings.darkMode ? 'Dark' : 'Light') + " Mode"}
        ?aria-pressed=${settings.darkMode}
        @click=${misc.toggleDarkMode}
        id=dark_mode_toggle>
        </button>
    ...
    <div class=configuration id=advancedUI_toggle>
        ...
        data-icon
        id="advanced_toggle"
        ?aria-pressed=${settings.advancedUI}
        title=${(settings.advancedUI ? 'Disable' : 'Enable') + " Advanced UI"}
        @click=${misc.toggleAdvancedUI}>
        </button>
    ...
    <div class=configuration id=manageTemplates>
        ...
        <!-- Select Template -->
    ${State.templates.length ? html`
        <select 
        title=${"Current Template: " + State.now.selected.template.name} 
        @input=${loadTemplate}>
    ...
    ${State.templates.map(template => {
        ...
        data-icon
        title="Edit Template"
        @click=${renderModal.bind(this, 'template', true)}>
        </button>
        ...
        data-icon
        title="New Template"
        @click=${renderModal.bind(this, 'template', false)}>
        </button>
    ...
    <div class=configuration id=import_export>
        ...
        title="Export Data"
        id=exportData
        .href=${State?.forExport}
        download=${`My Data (${new Date(State.lastSaved).toLocaleDateString()}).json`}
        >
        </a>
